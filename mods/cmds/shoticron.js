...
const shoticron = async (api, event, threadID) => {
  try {
    ...
    const file = fs.createWriteStream('shoti-jane.mp4');
    const rqs = request(encodeURI(response.data.data.url));
    rqs.pipe(file);

    file.on('finish', () => {
      api.sendMessage({
        body: `𝚃𝚑𝚒𝚜 𝚒𝚜 𝚊𝚞𝚝𝚘𝚖𝚊𝚝𝚎𝚍 𝚜𝚎𝚗𝚍𝚒𝚗𝚐 𝚜𝚑𝚘𝚝𝚒 video🌹\n\n\n𝚃𝚒𝚝𝚕𝚎: ${title}\n\n𝙽𝚒𝚌𝚔𝚗𝚊𝚖𝚎: ${nickname}\n\n𝚄𝚜𝚎𝚛𝚗𝚊𝚖𝚎: ${username}\n\n𝙳𝚞𝚛𝚊𝚝𝚒𝚘𝚗𝚜: ${durations}\n\n𝙳𝚊𝚝𝚎 𝚊𝚗𝚍 𝚃𝚒𝚖𝚎: ${time}`,
        attachment: fs.createReadStream('shoti-jane.mp4'),
      }, threadID, () => {
        fs.unlink('shoti-jane.mp4', (err) => {
          if (err) {
